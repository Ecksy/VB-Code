...
Dim sIp, sPlugin, sCVSS, sRisk, sSubnet, sOc4, sIP2, sDesc, sRem, sBase1, sBase2, sBase3, sSlash1, sPort, sPort2 As String
Dim iPlugStart, iPlugEnd, iFootprint, iAddDNS, iDNSEOF, iPortStart, iPortEnd As Integer
...
Private Sub SortPorts()
    For i = 1 To colPlugins.Count
        ...
        'get last row containing port number
        x = 4
        For Each cell In Range("A4:A65535")
            If cell.Value = "" Then Exit For
            x = x + 1
        Next cell
        ...
        'remove duplicates
        Range("A4:A" & x).Select
        ActiveSheet.Range("$A$4:$A$" & x).RemoveDuplicates Columns:=1, Header:=xlNo
        
        'get new last row containing port numbers after duplicates are removed
        x = 4
        For Each cell In Range("A4:A65535")
            If cell.Value = "" Then Exit For
            x = x + 1
        ...
        'sort port numbers
        Range("A4:A" & x - 1).Select
        ActiveSheet.Sort.SortFields.Clear
        ActiveSheet.Sort.SortFields.Add2 Key:=Range("A4"), _
            SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortTextAsNumbers
        With ActiveSheet.Sort
            .SetRange Range("A4:A" & x)
            .Header = xlNo
            .MatchCase = False
            .Orientation = xlTopToBottom
            .SortMethod = xlPinYin
...
Private Sub PopulateTable()
    ...
    For i = 1 To colPlugins.Count
        ...
        'concatenate port numbers
        z = 4
        For z = 4 To 32767
            If Range("A" & z).Value = "" Then Exit For
            If Range("A3").Value = "" Then
                Range("A" & 3).Value = Range("A" & z).Value
            Else
                Range("A" & 3).Value = Range("A" & 3).Value & ", " & Range("A" & z).Value
            End If
        Next z
...
Private Sub PlugData(ByVal colIP As Collection)
    ...
    Range("D" & x).Value = sRisk
    Range("E" & x).Value = sCVSS
    Range("G" & x).Value = sPort2
...
For Each cell In Range("A:A")
    ...
    'get port number
    If (InStr(1, cell.Value, "ReportItem", vbTextCompare) > 0) And (InStr(1, cell.Value, "port=", vbTextCompare) > 0) Then
        iPortStart = InStr(1, cell.Value, "port=", vbTextCompare) + 6
        iPortEnd = InStr(iPortStart + 1, cell.Value, """", vbTextCompare)
        sPort = Mid(cell.Value, iPortStart, iPortEnd - iPortStart)
    End If
...
Private Sub OrderData()
    ...
    ActiveWorkbook.Worksheets("VulnReported").AutoFilter.Sort.SortFields.Add Key:=Range _
        ("E1:E" & x), SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:= _
        xlSortTextAsNumbers
...
Private Sub ReformatNOWP(ByRef wdoc, ByRef sEng, ByRef WordContent)
    'Replace the word bank with {type}
    ...
    'Remove unnecessary superscript note explanations at the end of the reports
    wdoc.ActiveWindow.Selection.Find.Text = "Notes:"
    wdoc.ActiveWindow.Selection.Find.Execute
    wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=1
    Select Case sEng
        Case "IPT", "EPT", "EVS"
            wdoc.ActiveWindow.Selection.MoveDown Unit:=wdLine, Count:=3, Extend:=wdExtend
        Case "SE", "PA"
            wdoc.ActiveWindow.Selection.MoveDown Unit:=wdLine, Count:=2, Extend:=wdExtend
    End Select
    ...
    Select Case sEng
        ...
        Case "IPT"
            ...
            wdoc.ActiveWindow.Selection.TypeBackspace
            wdoc.ActiveWindow.Selection.TypeBackspace
            'change statement about providor of IPs back to tech contact
            wdoc.ActiveWindow.Selection.Find.Text = "The table below identifies the addresses included and were verified by {contact_name}"
            wdoc.ActiveWindow.Selection.Find.Execute
            wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=1
            wdoc.ActiveWindow.Selection.Find.Text = "{contact_name}"
            wdoc.ActiveWindow.Selection.Find.Execute
            wdoc.ActiveWindow.Selection.TypeText Text:="{tech_contact}"
        Case "EPT"
            'remove statement that is likely never going to be used
            wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
            wdoc.ActiveWindow.Selection.Find.Text = "This engagement is a dual-purpose engagement, covering both the FFIEC regulatory procedures and it will be used by the financial auditors of BKD in the performance of the external financial statement audit.(A)"
...
Private Sub ImportBKDMan(ByRef wdoc, ByRef sEng)
    ...
    wdoc.ActiveWindow.Selection.Find.Text = "[Insert report from BKDMAN"
    wdoc.ActiveWindow.Selection.Find.Execute
    wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=1
    If sEng = "SE" Or sEng = "PA" Then
        wdoc.ActiveWindow.Selection.MoveDown Unit:=wdLine, Count:=5, Extend:=wdExtend
    Else
        wdoc.ActiveWindow.Selection.MoveDown Unit:=wdLine, Count:=1, Extend:=wdExtend
    End If
    wdoc.ActiveWindow.Selection.TypeBackspace
    ...
    Select Case sEng
        Case "IPT"
            wdoc.ActiveWindow.Selection.Find.Text = "[Insert Section]"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                wdoc.ActiveWindow.Selection.TypeText Text:="Section VI"
            ...
            wdoc.ActiveWindow.Selection.Find.Text = "[specify – internal control over information technology systems OR internal network security OR social engineering awareness]"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                wdoc.ActiveWindow.Selection.TypeText Text:="internal network security"
            ...
            wdoc.ActiveWindow.Selection.Find.Text = "(specify - findings, recommendations and observations)"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                wdoc.ActiveWindow.Selection.TypeText Text:=" recommendations and observations"
            wdoc.ActiveWindow.Selection.Find.Text = "[Insert Section]"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                wdoc.ActiveWindow.Selection.TypeText Text:=" Section VI"
        ...
        Case "EPT"
            wdoc.ActiveWindow.Selection.Find.Text = "[Insert Section]"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                wdoc.ActiveWindow.Selection.TypeText Text:="Section IV"
            wdoc.ActiveWindow.Selection.Find.Text = "[Client Name]"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                wdoc.ActiveWindow.Selection.TypeText Text:=Range("A2").Value
        ...
        Case "EVS"
            ...
            wdoc.ActiveWindow.Selection.Find.Text = "[Insert Section]"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                wdoc.ActiveWindow.Selection.TypeText Text:="Section V"
            wdoc.ActiveWindow.Selection.Find.Text = "[Select one of the following sentences – "
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
            wdoc.ActiveWindow.Selection.Find.Text = "(specify - findings, recommendations and observations)"
    ...
    wdoc.ActiveWindow.Selection.Find.Text = "[Date]"
        wdoc.ActiveWindow.Selection.Find.Execute
        wdoc.ActiveWindow.Selection.TypeBackspace
        wdoc.ActiveWindow.Selection.TypeText Text:=Range("A6").Text
End Sub

Private Sub WriteReport(ByVal sEng As String)
    Dim sReportLocation As String
    sReportLocation = Range("D1").Value
    If InStr(Len(sReportLocation), sReportLocation, "\", vbTextCompare) = 0 Then sReportLocation = sReportLocation & "\"
    sReportLocation = sReportLocation & Range("D3").Value
    sReportLocation = FileSelectBox("Pfx Report Template", "*.docx", "Select PFx Report to Import", sReportLocation)
    Call DeleteTempSheets("TMP")
    If sEng = "IPT" Then Call WriteFindings
    Sheets("Control").Select
    Dim observation, recommendation, ipaddress, sObPorts, sObRisk, cvss, FilePath, sType, sClient, sAddress, sElDate, sDateFirst, sDateLast, sTempVulnSheet As String
    ...
    For Each cell In Range("B2:B32767")
        ...
        With WordContent.Find
            ...
            If Range("B" & cell.Row).Value = "{today}" Then
                .Replacement.Text = Format(Range("A" & cell.Row).Value, "MMMM D, YYYY")
    ...
    If sEng = "EPT" Or sEng = "EVS" Or sEng = "IPT" Then
        wdoc.ActiveWindow.Selection.Find.Text = "[Insert Addresses]"
        wdoc.ActiveWindow.Selection.Find.Execute
        wdoc.ActiveWindow.Selection.Font.Italic = False
        wdoc.ActiveWindow.Selection.Font.Underline = wdUnderlineNone
        wdoc.ActiveWindow.Selection.TypeBackspace
        wdoc.ActiveWindow.Selection.TypeText Text:=Worksheets("Control").txtIPRanges.Text
    End If
    
    If ThisWorkbook.Sheets("Control").chkLegacyObservations.Value = True Then
        'Write observations for pen tests only
        If sEng = "EPT" Or sEng = "EVS" Or sEng = "IPT" Then
        
            'Change placeholders for EPT and EVS to avoid confusion relating to findings and observations in other parts
            If sEng = "EPT" Or sEng = "EVS" Then
                With WordContent.Find
                    .Text = "{finding"
                    .Replacement.Text = "{observation"
                    .Wrap = wdFindContinue
                    .Execute Replace:=wdReplaceAll
                End With
                With WordContent.Find
                    .Text = "{ipaddress"
                    .Replacement.Text = "{obs_ipaddress"
                    .Wrap = wdFindContinue
                    .Execute Replace:=wdReplaceAll
                End With
                With WordContent.Find
                    .Text = "{recommendation"
                    .Replacement.Text = "{obs_recommendation"
                    .Wrap = wdFindContinue
                    .Execute Replace:=wdReplaceAll
                End With
            End If
            ...
            For Each cell In Range("A:A")
                ...
                ipaddress = Range("f" & x).Value
                cvss = Range("e" & x).Value
                sObRisk = Range("d" & x).Value
                sObPorts = Range("g" & x).Value
                If cvss = "" Then Exit For
                
                'Write observations for IPT Appendix and findings for EPT and EVS
                wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
                ...
                'Write cvss risk rating for IPT appendix
                If sEng = "IPT" Then
                    wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
                    wdoc.ActiveWindow.Selection.Find.Text = "{cvss_risk" & x - 1 & "}"
                    wdoc.ActiveWindow.Selection.Find.Execute
                    wdoc.ActiveWindow.Selection.TypeBackspace
                    wdoc.ActiveWindow.Selection.TypeText Text:=sObRisk
                End If
                
                'Write CVSS score with risk rating for EPT and EVS, and write CVSS score for IPT appendix
                wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
                wdoc.ActiveWindow.Selection.Find.Text = "{cvss" & x - 1 & "}"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                If sEng = "IPT" Then
                    wdoc.ActiveWindow.Selection.TypeText Text:=CStr(cvss)
                Else
                    wdoc.ActiveWindow.Selection.TypeText Text:=sObRisk & "/" & CStr(cvss)
                End If
                If InStr(1, CStr(cvss), ".", vbTextCompare) = 0 Then cvss = cvss & ".0"
                
                'Write port numbers to IPT appendix
                If sEng = "IPT" Then
                    wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
                    wdoc.ActiveWindow.Selection.Find.Text = "{port" & x - 1 & "}"
                    wdoc.ActiveWindow.Selection.Find.Execute
                    wdoc.ActiveWindow.Selection.TypeBackspace
                    wdoc.ActiveWindow.Selection.TypeText Text:=CStr(sObPorts)
                End If
                
                'write IP addresses
                wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
                wdoc.ActiveWindow.Selection.Find.Text = "{obs_ipaddress" & x - 1 & "}"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                ...
                wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
                wdoc.ActiveWindow.Selection.Find.Text = "{obs_recommendation" & x - 1 & "}"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
    ...
    'Write finding counts (legacy code switch)
    If ThisWorkbook.Sheets("Control").chkLegacyObservations.Value = True Then
        If sEng = "IPT" Then
            'PIM findings
            Sheets("Findings").Select
            ...
            For Each cell In Range("A2:A250")
                If cell <> "" Then x = x + 1
            Next cell
            wdoc.ActiveWindow.Selection.Find.Text = "The Bank had {total_findings} findings based on probability and impact, which are defined in Section III"
            wdoc.ActiveWindow.Selection.Find.Execute
            wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=1
            wdoc.ActiveWindow.Selection.Find.Text = "{total_findings}"
            wdoc.ActiveWindow.Selection.Find.Execute
            wdoc.ActiveWindow.Selection.TypeText Text:=ConvertSingleDigit(x)
            wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
            
            'cvss findings
            Sheets("VulnReported").Select
            x = 0
            For Each cell In Range("B2:B250")
                If cell <> "" Then x = x + 1
            Next cell
            wdoc.ActiveWindow.Selection.Find.Text = "The Bank had {total_findings} findings based on the Common Vulnerability Scoring System"
            wdoc.ActiveWindow.Selection.Find.Execute
            wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=1
            wdoc.ActiveWindow.Selection.Find.Text = "{total_findings}"
            wdoc.ActiveWindow.Selection.Find.Execute
            wdoc.ActiveWindow.Selection.TypeText Text:=ConvertSingleDigit(x)
            wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
        ElseIf sEng = "EPT" Or sEng = "EVS" Then
            Sheets("VulnReported").Select
            x = 0
            For Each cell In Range("B2:B250")
                If cell <> "" Then x = x + 1
            Next cell
            wdoc.ActiveWindow.Selection.Find.Text = "The Bank had a total of {total_findings} findings.  These are rated based on the Common Vulnerability Scoring System (CVSS)"
            wdoc.ActiveWindow.Selection.Find.Execute
            wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=1
            wdoc.ActiveWindow.Selection.Find.Text = "{total_findings}"
            wdoc.ActiveWindow.Selection.Find.Execute
    ...
    If sEng = "IPT" Then
        ...
        If z <> 0 Then
            For Each cell In Range("A" & x & ":A" & z)
                ...
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.TypeBackspace
                If sOverall = "Low" Then
                    wdoc.ActiveWindow.Selection.TypeText Text:="Low"
                ElseIf sOverall = "Medium" Then
                ...
                wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=Len(sProbability)
                wdoc.ActiveWindow.Selection.MoveLeft Extend:=wdExtend, Unit:=wdCharacter, Count:=Len("{probability" & x & "}")
                wdoc.ActiveWindow.Selection.Delete
                wdoc.ActiveWindow.Selection.Find.Text = "{impact" & x & "}"
                wdoc.ActiveWindow.Selection.Find.Execute
                wdoc.ActiveWindow.Selection.MoveRight Unit:=wdCharacter, Count:=1
                wdoc.ActiveWindow.Selection.TypeText Text:=sImpact
                wdoc.ActiveWindow.Selection.MoveLeft Unit:=wdCharacter, Count:=Len(sImpact)
                wdoc.ActiveWindow.Selection.MoveLeft Extend:=wdExtend, Unit:=wdCharacter, Count:=Len("{impact" & x & "}")
                wdoc.ActiveWindow.Selection.Delete
        ...
        'Update charts
        Call UpdateCharts(wdoc, sEng, "finding")
        Call UpdateCharts(wdoc, sEng, "observation")
    End If
...
Private Sub UpdateCharts(ByRef wdoc, ByRef sEng, ByVal sType As String)
    Dim iLow, iMedium, iHigh, iCritical, iStartRow As Integer
    Dim sColLoop, sColCheck, sColData, sChart, sSearch
    iLow = 0
    iMedium = 0
    iHigh = 0
    iCritical = 0
    
    'determine columns and select worksheet
    If sEng = "IPT" Then
        If sType = "finding" Then
            ...
            iStartRow = 3
            sChart = "Chart 1"
            sSearch = "findings based on probability and impact"
        ElseIf sType = "observation" Then
            Sheets("VulnReported").Select
            sColLoop = "A"
            sColCheck = "A"
            sColData = "D"
            iStartRow = 22
            sChart = "Chart 2"
            sSearch = "findings based on the Common Vulnerability Scoring System"
        End If
    End If
    
    'gather counts
    For Each cell In Range(sColLoop & ":" & sColLoop)
        If cell.Value = "" Then Exit For
        If Range(sColCheck & cell.Row).Value <> "" Then
            Select Case Range(sColData & cell.Row).Value
                Case "Critical"
    ...
    Range("B" & iStartRow).Value = iCritical
    Range("B" & iStartRow + 1).Value = iHigh
    Range("B" & iStartRow + 2).Value = iMedium
    Range("B" & iStartRow + 3).Value = iLow
    
    'update charts
    ActiveSheet.ChartObjects(sChart).Activate
    ActiveChart.ChartArea.Copy
    wdoc.ActiveWindow.Selection.HomeKey Unit:=wdStory
    wdoc.ActiveWindow.Selection.Find.Text = sSearch
    wdoc.ActiveWindow.Selection.Find.Execute
    wdoc.ActiveWindow.Selection.MoveRight
    wdoc.ActiveWindow.Selection.Find.Text = "[Insert Graph]"
    wdoc.ActiveWindow.Selection.Find.Execute
    wdoc.ActiveWindow.Selection.TypeBackspace
    wdoc.ActiveWindow.Selection.Paste
    wdoc.ActiveWindow.Selection.ParagraphFormat.Alignment = wdAlignParagraphCenter
End Sub
...
Function FileSelectBox(ByRef sFile As String, ByRef FileType As String, ByRef sTitle As String, Optional ByVal DefaultDir As String) As String
...
Private Sub WriteChecklists(ByVal sEng As String, ByVal sCyber As String, ByVal sDate As String)
    ...
    ElseIf sEng = "MCS0501 Consulting Report Release Checklist.xlsm" Then
        ...
        If ThisWorkbook.Sheets("Control").chkFinalOnly.Value = True Then If sDate <> "" Then xlWorkbook.Worksheets("Report Release").Range("G16").Value = Format(CDate(Range("A12").Value), "mm/dd/yyyy")
...
Sub PickPhishingFile()
    ...
    d = ""
    Range("I13").Formula = "='" & Left(x, Len(x) - Len(y)) & "[" & y & "]Control'!$G$2"
    d = d & Range("I13").Value & "/"
    Range("I13").Formula = "='" & Left(x, Len(x) - Len(y)) & "[" & y & "]Control'!$G$3"
    d = d & Range("I13").Value & "/"
    Range("I13").Formula = "='" & Left(x, Len(x) - Len(y)) & "[" & y & "]Control'!$G$4"
    d = d & Range("I13").Value & "?"
    Range("I13").Formula = "='" & Left(x, Len(x) - Len(y)) & "[" & y & "]Control'!$G$5"
    d = d & Range("I13").Value & "=1"
    d = "http://www." & d
    Range("I13").Value = d
...
Sub CustomMailMessage(ByVal sEng As String)
    ...
    objOutlookMsg.HTMLBody = "<html><body style=font-size:10pt;font-family:Arial>Chuck:" & "<br/><br/>" & _
        ...
        "Pfx Client: <strong>" & CStr(Range("A2").Value & " (" & CStr(Range("A11").Value) & ") " & "</strong><br/>" & _
        "Pfx Binder ID: <strong>PT" & " " & CStr(Year(Range("A6").Value)) & "</strong><br/>" & _
        "Pfx Binder tab: <strong>" & Range("A10").Value & "</strong><br/>" & _
...
Private Function ConvertSingleDigit(ByVal x As Integer) As String
    If x < 10 Then
        Select Case x
            Case 0
                ConvertSingleDigit = "zero"
            Case 1
                ConvertSingleDigit = "one"
            Case 2
                ConvertSingleDigit = "two"
            Case 3
                ConvertSingleDigit = "three"
            Case 4
                ConvertSingleDigit = "four"
            Case 5
                ConvertSingleDigit = "five"
            Case 6
                ConvertSingleDigit = "six"
            Case 7
                ConvertSingleDigit = "seven"
            Case 8
                ConvertSingleDigit = "eight"
            Case 9
                ConvertSingleDigit = "nine"
        End Select
    Else
        ConvertSingleDigit = CStr(x)
    End If
End Function
